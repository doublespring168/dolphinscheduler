...
FROM r.gyykj.com/gyy-base/openeuler-jdk:1.8-amd64-r AS gyyun-dist
...
FROM r.gyykj.com/gyy-base/openeuler-jdk:1.8-amd64-r
...
RUN chown -R gyy /gyy_workspace && \
    ...
    chmod +x -R /gyy_program/api-server/bin

USER gyy

EXPOSE 12345

CMD [ "/bin/sh", "/gyy_program/api-server/bin/start.sh" ]
