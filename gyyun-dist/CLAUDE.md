# CLAUDE.md — gyyun-dist

The **release assembler**. Produces the binary tarball (`apache-gyyun-<version>-bin.tar.gz`) and the source tarball, optionally a Docker image. Has no Java code — it is a Maven assembly + shell-script orchestration module.

## Artifacts produced

- `apache-gyyun-${version}-bin.tar.gz` — standalone-server + master + worker + api + alert + UI + tools + plugin jars + scripts + configs.
- `apache-gyyun-${version}-src.tar.gz` — ASF-compliant source tarball.
...
- `gyyun-bin.xml` — file layout of the binary tarball.
- `gyyun-src.xml` — file layout of the source tarball.
...
./mvnw -pl gyyun-dist -am clean package

# With Docker images
./mvnw -pl gyyun-dist -am -P docker clean package
```

Output lands in `gyyun-dist/target/`.
...
- **UI is built inside `gyyun-ui`** (Node / pnpm) during `./mvnw package`; this module picks up the `dist/` folder. A broken UI build breaks the dist build.
...
None. Verification is end-to-end via `gyyun-e2e` which runs against a built tarball.

## Related modules

- Dependencies at package-time: `gyyun-standalone-server`, `-api`, `-alert-server`, `-ui`, `-tools`.
