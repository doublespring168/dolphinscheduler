...
```
pnpm install
pnpm run dev         # Vite dev server (default :5173), proxies /gyyun → backend
...
The backend URL used by `pnpm run dev` is `VITE_APP_DEV_WEB_URL` in `.env.development`. Default expects `gyyun-api` on port 12345.
...
- Axios base URL: `/gyyun` in dev (proxied), `VITE_APP_PROD_WEB_URL` in prod (usually same-origin behind a reverse proxy).
...
- **No unit tests** are configured (no `*.spec.ts` / `*.test.ts` found). End-to-end coverage comes from `gyyun-e2e`.
...
- **The built `dist/` is what `gyyun-dist` picks up**. Ensure `pnpm run build:prod` has been run before packaging from the repo.

## Tests

None inside this module. See `gyyun-e2e` for Selenium-driven browser tests.

## Related modules

- `gyyun-api` — the backend this UI calls.
- `gyyun-dist` — bundles `dist/` into the tarball under `ui/`.
- `gyyun-e2e` — tests the integrated UI+API.
