...
- **`gyyun-alert-server`** — the runnable alert server. `@SpringBootApplication` `AlertServer` + HA coordinator (`AlertHAServer`) + RPC server (`AlertRpcServer`) + event loop.
- **`gyyun-alert-plugins`** — parent of the concrete channel plugins:
  - `gyyun-alert-email`, `-http`, `-feishu`, `-dingtalk`, `-wechat`, `-webexteams`, `-pagerduty`, `-telegram`, `-slack`, `-script`, etc. (see directory listing for the current set). Each plugin implements the `AlertPlugin` SPI and is loaded dynamically at runtime.

## How alerting works (end-to-end)

1. Master/worker calls into `gyyun-extract-alert` RPC with an `AlertRequest`.
...
- **`AlertPlugin` SPI** — new channel? Implement the interface in a new sub-module of `gyyun-alert-plugins` and it will be discovered by `AlertPluginManager`.
...
- `gyyun-extract-alert` — RPC contracts this server implements.
- `gyyun-dao` — persists alert events + audit.
- `gyyun-registry-all`, `gyyun-meter`, `gyyun-spi`.
- Callers: `gyyun-master`, `gyyun-worker`.
