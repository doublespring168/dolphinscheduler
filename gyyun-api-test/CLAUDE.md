...
**This is a Maven parent POM.** This module is **not** declared in the root `pom.xml` `<modules>` — run it explicitly (e.g. `mvn -pl gyyun-api-test test -am`) or via CI workflows.

## Sub-modules

- **`gyyun-api-test-core`** — the reusable framework:
...
- **`gyyun-api-test-case`** — the actual test classes. `WorkflowDefinitionAPITest`, `TenantAPITest`, `SchedulerAPITest`, etc. Each class uses `@DolphinScheduler(composeFiles = { "docker/basic/docker-compose.yaml" })` to declare its environment.
...
- `gyyun-api` — the system under test.
- `gyyun-dao` — depended on to reuse entity DTOs.
- `gyyun-e2e` — complementary browser-level tests (selenium against UI).
