...
if [ ! -x /opt/gyyun/standalone-server/bin/start.sh ]; then
  echo "start.sh not executable. Applying chmod +x ..."
  chmod +x /opt/gyyun/standalone-server/bin/start.sh || {
    echo "Failed to chmod start.sh" >&2
    ls -l /opt/gyyun/standalone-server/bin/start.sh || true
    exit 1
  }
fi
...
/opt/gyyun/standalone-server/bin/start.sh
