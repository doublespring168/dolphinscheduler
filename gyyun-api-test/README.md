...
1.  **Build the image**: From the root of the project, run the following command. This creates a standalone server image tagged as `ci`.
    ...
    ```
2.  **Verify the image**: Ensure the image `apache/gyyun-standalone-server:ci` was created successfully.
    ```bash
    docker images | grep "gyyun-standalone-server"
    ```
...
1.  Ensure the `gyyun-api-test` module is included in your Maven projects in your IDE.
...
* **To modify DolphinScheduler's configuration**: Edit the `environment` section for the `gyyun` service in `docker/oidc-login/docker-compose.yaml` to change OIDC settings or other parameters.
