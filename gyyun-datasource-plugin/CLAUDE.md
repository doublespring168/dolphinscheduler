# CLAUDE.md — gyyun-datasource-plugin
...
- **`gyyun-datasource-api`** — SPI and base types: `DataSourceProcessor`, `AbstractDataSourceProcessor`, `BaseDataSourceParamDTO`, `BaseHdfsConnectionParam`, and the `DataSourcePluginManager` that loads plugins.
- **`gyyun-datasource-all`** — uber module bundling every plugin; depended on by `master`, `worker`, `api` so they can use any datasource at runtime.
...
1. `DataSourceChannelFactory` (from `gyyun-spi`, loaded via `PrioritySPIFactory`) — creates `DataSourceChannel`s for low-level connection handling.
...
- **`DbType` enum lives in `gyyun-spi`**. Adding a new datasource means: new enum value in spi + new plugin sub-module. Leaving the enum unchanged will cause `DataSourcePluginManager` to silently ignore the plugin.
...
- **JDBC connection pooling** is **Druid** (not HikariCP). This is different from the metadata DB (`gyyun-dao` uses HikariCP). Be careful with which you're tuning.
...
- `gyyun-spi` — base SPI (`DataSourceChannelFactory`, `DbType`).
- `gyyun-task-plugin` — primary consumer (SQL family plugins).
- `gyyun-api`, `gyyun-service` — consumers for configuration + validation.
- `gyyun-common` — `PasswordUtils`.
