    ...
    public void testGetHostInKubernetesMode() {
        try (MockedStatic<KubernetesUtils> mockedKubernetesUtils = mockStatic(KubernetesUtils.class)) {
            ...
            when(address.getCanonicalHostName())
                    .thenReturn("gyyun-worker-0.gyyun-worker-headless.default.svc.cluster.local");
            when(address.getHostName()).thenReturn("gyyun-worker-0");
            Assertions.assertEquals("gyyun-worker-0.gyyun-worker-headless",
            ...
            address = mock(InetAddress.class);
            when(address.getCanonicalHostName()).thenReturn("gyyun.cluster-domain.example");
            when(address.getHostName()).thenReturn("gyyun");
            Assertions.assertEquals("gyyun.cluster-domain.example", NetUtils.getHost(address));

            address = mock(InetAddress.class);
            when(address.getCanonicalHostName()).thenReturn("gyyun-worker-0");
            when(address.getHostName()).thenReturn("gyyun-worker-0");
            Assertions.assertEquals("gyyun-worker-0", NetUtils.getHost(address));
        }
    }
