# CLAUDE.md — gyyun-task-executor

Reusable task-execution framework. Defines how a worker **runs**, **tracks**, and **reports** a task instance — independent of any specific task type (shell, Spark, …). The worker (`gyyun-worker`) embeds this module; the task-type behavior comes from `gyyun-task-plugin`.
...
- `task.executor.eventbus` — `TaskExecutorEventBus` + `TaskExecutorEventBusCoordinator`; an in-process delay bus for task-lifecycle transitions (built on `gyyun-eventbus`).
- `task.executor.listener` — lifecycle listeners (start, finish, fail, timeout, kill).
- `task.executor.operations` — operation requests/responses carried over RPC (dispatch, kill, pause, reassign). Sibling to the wire types in `gyyun-extract-worker`.
...
Exposed interfaces (consumed by `gyyun-worker`):
...
- **No Spring here**. This module is a plain library with no `@Component` / `@Configuration`. All beans are wired inside `gyyun-worker`. Do not add Spring deps.
...
- **Module has no tests** — coverage comes from `gyyun-worker` integration tests. Changes here need worker-side verification.

## Related modules

- `gyyun-eventbus` — underlies the task-lifecycle bus.
- `gyyun-task-api` — task DTOs/contracts.
- `gyyun-common` — utilities.
- `gyyun-worker` — the one and only consumer today.
- `gyyun-master` — receives lifecycle events (via RPC) from this module's reporter.
