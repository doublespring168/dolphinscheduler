...
usage="Usage: gyyun-daemon.sh (start|stop|status) <api-server|master-server|worker-server|alert-server|standalone-server> "
...
BIN_ENV_FILE="${DOLPHINSCHEDULER_HOME}/bin/env/gyyun_env.sh"

# Overwrite server gyyun_env.sh in path `<server>/conf/gyyun_env.sh` when exists
# `bin/env/gyyun_env.sh` file. User could only change `bin/env/gyyun_env.sh` instead
# of each server's gyyun_env.sh when they want to start the server
function overwrite_server_env() {
  local server=$1
  local server_env_file="${DOLPHINSCHEDULER_HOME}/${server}/conf/gyyun_env.sh"
  if [ -f "${BIN_ENV_FILE}" ]; then
    echo "Overwrite ${server}/conf/gyyun_env.sh using bin/env/gyyun_env.sh."
