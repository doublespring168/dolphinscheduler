...
###
 ...
 # @LastEditTime: 2026-06-13 21:01:07
 # @FilePath: /ala-ds/script/install-plugins.sh
 # @Description: 
 # 
...
# plugins default version is 3.4.2, you can also choose a custom version. eg: 3.4.2: bash install-plugins.sh 3.4.2
version=3.4.2
...
while read line; do
  ...
  if [ "$start_char" != "-" ] && [ "$start_char" != "#" ]; then
      ...
      ${DOLPHINSCHEDULER_HOME}/mvnw dependency:get -DgroupId=com.gyyun.ds -DartifactId=${line} -Dversion=${version} -Dclassifier=shade -Ddest=${DOLPHINSCHEDULER_HOME}/plugins/${plugin_dir}
  fi
