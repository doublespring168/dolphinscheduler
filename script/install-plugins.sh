...
while read line; do
  ...
  if [ "$start_char" != "-" ] && [ "$start_char" != "#" ]; then
      ...
      ${DOLPHINSCHEDULER_HOME}/mvnw dependency:get -DgroupId=org.apache.gyyun -DartifactId=${line} -Dversion=${version} -Dclassifier=shade -Ddest=${DOLPHINSCHEDULER_HOME}/plugins/${plugin_dir}
  fi
