...
- **AspectJ version is pinned in `gyyun-bom`** (`aspectj.version`, currently 1.9.7). Upgrading AspectJ is sensitive — test every YARN-based task plugin.
...
- `gyyun-worker` — runtime consumer (this jar is on the worker's classpath).
- `gyyun-task-plugin` → `task-mr`, `task-spark`, `task-sqoop`, `task-hivecli` — the YARN-submitting task plugins that benefit from this.
