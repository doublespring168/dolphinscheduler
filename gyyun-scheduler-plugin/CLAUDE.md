# CLAUDE.md — gyyun-scheduler-plugin
...
- **`gyyun-scheduler-api`** — the `SchedulerApi` interface (`start`, `insertOrUpdateScheduleTask`, `deleteScheduleTask`, `close`) + related DTOs.
- **`gyyun-scheduler-quartz`** — Quartz-based implementation. `QuartzScheduler` (SchedulerApi impl), `QuartzSchedulerAutoConfiguration`, `QuartzSchedulerDataSourceAutoConfiguration`, `QuartzTriggerBuilder`.
- **`gyyun-scheduler-all`** — uber module consumed by master.
...
- **Cron parsing happens in two places**: `gyyun-service/cron` uses `cron-utils` for *display* (next fire time in UI); Quartz internally uses Quartz cron syntax for *firing*. The two are mostly compatible but DOW conventions differ slightly — always validate with both.
...
- `gyyun-master` — consumes `scheduler-all`.
- `gyyun-service` — uses `CronService` for next-fire-time display (independent of this module's runtime).
