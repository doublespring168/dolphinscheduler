...
2. If empty → **init** path: runs `sql/gyyun_<dialect>.sql` from `gyyun-dao/src/main/resources/sql/`.
...
- **Adding a release version** means adding a new `V<version>DolphinSchedulerUpgrader` plus any DDL under `gyyun-dao/src/main/resources/sql/upgrade/<version>/`. Skipping either half silently produces a broken upgrade.
...
- **Do not reach into `gyyun-dao` entities from upgraders**. Upgrades run against older schemas where those entities may not map; use raw SQL through the dialect-aware helpers.
...
- **Shell scripts source `gyyun_env.sh`** for env vars; when debugging, check whether the expected JDBC URL is exported.
...
- `gyyun-dao` — source of SQL scripts + entities (read-only here).
- `gyyun-storage-all` — needed by `MigrateResource` to read/write stored artifacts.
- `gyyun-dao-plugin` — dialect selection.
- `gyyun-dist` — packages the tool jar + scripts into the tarball.
