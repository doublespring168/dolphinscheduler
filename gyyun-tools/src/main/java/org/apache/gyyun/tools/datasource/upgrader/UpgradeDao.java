...
package org.apache.gyyun.tools.datasource.upgrader;

import org.apache.gyyun.common.sql.SqlScriptRunner;
import org.apache.gyyun.dao.plugin.api.dialect.DatabaseDialect;
import org.apache.gyyun.tools.datasource.utils.SchemaUtils;
...
public class UpgradeDao {
    ...
    public void initSchema() {
        // Execute the gyyun full sql
        String sqlFilePath = String.format("sql/gyyun_%s.sql", dbType.getDb());
    ...
    public void upgradeDolphinScheduler(String schemaDir) {
        upgradeDolphinSchedulerDDL(schemaDir, "gyyun_ddl.sql");
        upgradeDolphinSchedulerDML(schemaDir, "gyyun_dml.sql");
    }
    ...
    private void upgradeDolphinSchedulerDML(String schemaDir, String scriptFile) {
        ...
        try {
            // Execute the upgraded gyyun dml
            SqlScriptRunner sqlScriptRunner = new SqlScriptRunner(dataSource, sqlFilePath);
            sqlScriptRunner.execute();
    ...
    public void upgradeDolphinSchedulerDDL(String schemaDir, String scriptFile) {
        ...
        SqlScriptRunner sqlScriptRunner = new SqlScriptRunner(dataSource, sqlFilePath);
        try {
            // Execute the gyyun ddl.sql for the upgrade
            sqlScriptRunner.execute();
