# CLAUDE.md — gyyun-storage-plugin
...
- **`gyyun-storage-api`** — SPI: `StorageOperator`, `StorageOperatorFactory`, `AbstractStorageOperator`, `StorageType`, `StorageConfiguration`.
- **`gyyun-storage-all`** — uber bundle for all implementations.
...
- **Credentials**: cloud plugins use the SDK default chain when not explicitly configured. In AWS plugins, prefer IAM instance profile over static keys (see `gyyun-aws-authentication`).
...
- `gyyun-aws-authentication` — AWS plugins' credential source.
- `gyyun-common` — utilities.
- `gyyun-worker`, `gyyun-api`, `gyyun-master` — runtime consumers.
