...
package org.apache.gyyun.plugin.storage.obs;
...
public class ObsStorageOperatorTest {
    ...
    public void testListStorageEntity_withFilesAndDirectories() {
        ObsObject fileObject = createObsObject("tmp/gyyun/default/resources/demo.sql", 1024L);
        ObsObject dirSelfObject = createObsObject("tmp/gyyun/default/resources/", 0L);

        ObjectListing objectListing = mockObjectListing(
                Arrays.asList(dirSelfObject, fileObject),
                Arrays.asList("tmp/gyyun/default/resources/subDir1/",
                        "tmp/gyyun/default/resources/subDir2/"));

        when(mockObsClient.listObjects(any(ListObjectsRequest.class))).thenReturn(objectListing);

        List<StorageEntity> result =
                obsStorageOperator.listStorageEntity("tmp/gyyun/default/resources");
    ...
    public void testListStorageEntity_onlyFiles() {
        ObsObject fileObject = createObsObject("tmp/gyyun/default/resources/test.sql", 512L);
    ...
    public void testListStorageEntity_onlyDirectories() {
        ObsObject dirSelf = createObsObject("tmp/gyyun/default/resources/", 0L);

        ObjectListing objectListing = mockObjectListing(
                Collections.singletonList(dirSelf),
                Collections.singletonList("tmp/gyyun/default/resources/dir1/"));
