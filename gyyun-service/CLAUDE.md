...
- **Depends on RPC interfaces, not implementations**: this module pulls `gyyun-extract-master` / `-extract-worker` — it can *call* master/worker RPCs but does not implement them. The real implementations live in those server modules.
...
- `gyyun-dao` — primary dependency.
- `gyyun-spi`, `gyyun-registry-api` — plugin + service discovery.
- `gyyun-extract-master` / `-extract-worker` — RPC clients.
- `gyyun-task-api` — task-related DTOs.
- Consumers: `gyyun-api`, `gyyun-master`.
