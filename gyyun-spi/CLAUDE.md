...
- **Minimal dependencies on purpose**: only `slf4j-api` is `compile`; `gyyun-common` is `provided`. Resist the urge to add rich deps here — plugin authors consume this module transitively and each dep forces a version on them.
...
- `gyyun-common` — consumed `provided` only.
- `gyyun-task-plugin` / `gyyun-datasource-plugin` / `gyyun-storage-plugin` / `gyyun-dao-plugin` / `gyyun-scheduler-plugin` — all implement SPIs rooted here.
