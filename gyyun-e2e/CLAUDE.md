...
End-to-end **browser** tests. Selenium drives a real Chrome instance against a full DolphinScheduler stack booted with Docker Compose via Testcontainers. Complements the REST-level tests in `gyyun-api-test`.

**This is a Maven parent POM.** Like `gyyun-api-test`, it is **not** declared in the root `pom.xml` `<modules>` — run it explicitly or via dedicated CI.

## Sub-modules

- **`gyyun-e2e-core`** — the framework:
...
- **`gyyun-e2e-case`** — the test classes (`*E2ETest`) using page objects. One class per feature area.
...
- **`@Order` is mandatory** — tests assume sequential state (just like `gyyun-api-test`).
...
- `gyyun-ui` — the frontend under test.
- `gyyun-api` — the backend under test.
- `gyyun-api-test` — sibling test harness at the REST layer.
- `gyyun-dist` — produces the tarball some Compose scenarios install.
