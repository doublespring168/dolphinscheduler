...
- `api.controller` — 30+ `@RestController` classes, one per domain (workflows, tasks, users, projects, tenants, resources, data sources, alerts, monitoring, …). All URLs rooted at `/gyyun/*`.
- `api.service` / `api.service.impl` — business-logic layer wrapping `gyyun-service` and adding API-level concerns (auth checks, DTO mapping).
...
- `swagger.properties` — springdoc config (UI lives at `/gyyun/swagger-ui/index.html`).
- `logback-spring.xml` — logging.

## Gotchas

- **Jetty, not Tomcat**. `spring-boot-starter-tomcat` is excluded transitively via `gyyun-meter`. Avoid accidentally pulling Tomcat in.
...
Unit tests live in `src/test/java`. **Integration tests live in `gyyun-api-test`** (separate module, Docker Compose + Testcontainers).
...
- **`gyyun-bom` must be in the install `-pl`** — without it, the installed pom lacks effective dependencyManagement and downstream compile/test classpaths drop transitive deps like `commons-collections4` and `oshi-core`.
...
- `gyyun-service`, `gyyun-dao` — primary deps.
- `gyyun-extract-master` / `-worker` / `-alert` — RPC into servers.
- `gyyun-authentication` (actuator sub-module) — secures `/actuator/**`.
- `gyyun-ui` — primary consumer.
- `gyyun-api-test` — integration harness.
