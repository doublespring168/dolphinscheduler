...
- `microbench.rpc.RpcBenchMarkTest` — Netty-based RPC ping/pong benchmarks, exercises `gyyun-extract-base`.
...
```
# Build the uber-jar
./mvnw -pl gyyun-microbench -am package

# Run (Main-Class: org.openjdk.jmh.Main)
java -jar gyyun-microbench/target/benchmarks.jar [regex]
```
...
- `gyyun-extract-base` — the RPC framework under benchmark.
