...
| alert.customizedConfig | object | `{}` | configure aligned with https://github.com/apache/gyyun/blob/dev/gyyun-alert/gyyun-alert-server/src/main/resources/application.yaml |
...
| api.customizedConfig | object | `{}` | configure aligned with https://github.com/apache/gyyun/blob/dev/gyyun-api/src/main/resources/application.yaml |
...
| api.service.serviceMonitor | object | `{"annotations":{},"enabled":false,"interval":"15s","labels":{},"path":"/gyyun/actuator/prometheus"}` | serviceMonitor for prometheus operator |
...
| api.service.serviceMonitor.path | string | `"/gyyun/actuator/prometheus"` | serviceMonitor.path path of the metrics endpoint |
...
| api.taskTypeFilter.task | object | `{}` | ref: [task-type-config.yaml](https://github.com/apache/gyyun/blob/dev/gyyun-api/src/main/resources/task-type-config.yaml) |
...
| common.configmap.DATA_BASEDIR_PATH | string | `"/tmp/gyyun"` | User data directory path, self configuration, please make sure the directory exists and have read write permissions |
| common.configmap.DOLPHINSCHEDULER_OPTS | string | `""` | The jvm options for gyyun, suitable for all servers |
...
| common.configmap.RESOURCE_UPLOAD_PATH | string | `"/gyyun"` | Resource store on HDFS/S3 path, please make sure the directory exists on hdfs and have read write permissions |
...
| conf.common."aws.s3.bucket.name" | string | `"gyyun"` | The name of the bucket. You need to create them by yourself. Otherwise, the system cannot start. All buckets in Amazon S3 share a single namespace; ensure the bucket is given a unique name. |
...
| conf.common."data.basedir.path" | string | `"/tmp/gyyun"` | user data local directory path, please make sure the directory exists and have read write permissions |
...
| conf.common."ml.mlflow.preset_repository" | string | `"https://github.com/apache/gyyun-mlflow"` | mlflow task plugin preset repository |
...
| conf.common."resource.alibaba.cloud.oss.bucket.name" | string | `"gyyun"` | oss bucket name, required if you set resource.storage.type=OSS |
...
| conf.common."resource.hdfs.fs.defaultFS" | string | `"hdfs://mycluster:8020"` | if resource.storage.type=S3, the value like: s3a://gyyun; if resource.storage.type=HDFS and namenode HA is enabled, you need to copy core-site.xml and hdfs-site.xml to conf dir |
...
| conf.common."resource.storage.upload.base.path" | string | `"/gyyun"` | resource store on HDFS/S3 path, resource file will store to this base path, self configuration, please make sure the directory exists on hdfs and have read write permissions. "/gyyun" is recommended |
...
| externalDatabase.database | string | `"gyyun"` | The database of external database |
...
| image.alert | string | `"gyyun-alert-server"` | alert-server image |
| image.api | string | `"gyyun-api"` | api-server image |
| image.master | string | `"gyyun-master"` | master image |
...
| image.tools | string | `"gyyun-tools"` | tools image |
| image.worker | string | `"gyyun-worker"` | worker image |
| ingress.annotations | object | `{}` | Ingress annotations |
| ingress.enabled | bool | `false` | Enable ingress |
| ingress.host | string | `"gyyun.org"` | Ingress host |
| ingress.path | string | `"/gyyun"` | Ingress path |
| ingress.tls.enabled | bool | `false` | Enable ingress tls |
| ingress.tls.secretName | string | `"gyyun-tls"` | Ingress tls secret name |
| initImage | object | `{"busybox":"busybox:1.30.1","pullPolicy":"IfNotPresent"}` | Used to detect whether gyyun dependent services such as database are ready |
...
| master.customizedConfig | object | `{}` | configure aligned with https://github.com/apache/gyyun/blob/dev/gyyun-master/src/main/resources/application.yaml |
...
| minio.defaultBuckets | string | `"gyyun"` | minio default buckets |
...
| mysql.auth.database | string | `"gyyun"` | mysql database |
...
| postgresql.auth.database | string | `"gyyun"` | The database for internal PostgreSQL |
...
| registryEtcd.namespace | string | `"gyyun"` | Etcd namespace |
...
| worker.customizedConfig | object | `{}` | configure aligned with https://github.com/apache/gyyun/blob/dev/gyyun-worker/src/main/resources/application.yaml |
