...
*/}}
{{- define "gyyun.master.labels" -}}
app.kubernetes.io/name: {{ include "gyyun.fullname" . }}-master
app.kubernetes.io/component: master
{{ include "gyyun.common.labels" . }}
{{- end -}}

{{/*
Create a worker labels.
*/}}
{{- define "gyyun.worker.labels" -}}
app.kubernetes.io/name: {{ include "gyyun.fullname" . }}-worker
...
*/}}
{{- define "gyyun.alert.labels" -}}
app.kubernetes.io/name: {{ include "gyyun.fullname" . }}-alert
...
*/}}
{{- define "gyyun.api.labels" -}}
app.kubernetes.io/name: {{ include "gyyun.fullname" . }}-api
...
We truncate at 63 chars because some Kubernetes name fields are limited to this (by the DNS naming spec).
*/}}
{{- define "gyyun.postgresql.fullname" -}}
...
We truncate at 63 chars because some Kubernetes name fields are limited to this (by the DNS naming spec).
*/}}
{{- define "gyyun.zookeeper.fullname" -}}
...
*/}}
{{- define "gyyun.zookeeper.quorum" -}}
{{- $port := default "2181" .Values.zookeeper.service.port | toString -}}
{{- printf "%s:%s" (include "gyyun.zookeeper.fullname" .) $port -}}
{{- end -}}

{{/*
Create a database environment variables.
*/}}
{{- define "gyyun.database.env_vars" -}}
- name: DATABASE
...
- name: SPRING_DATASOURCE_URL
  {{- if .Values.postgresql.enabled }}
  value: jdbc:postgresql://{{ template "gyyun.postgresql.fullname" . }}:5432/{{ .Values.postgresql.auth.database }}?{{ .Values.postgresql.params }}
  {{- else if .Values.mysql.enabled }}
  value: jdbc:mysql://{{ template "gyyun.mysql.fullname" . }}:3306/{{ .Values.mysql.auth.database }}?{{ .Values.mysql.auth.params }}
  {{- else }}
...
- name: SPRING_DATASOURCE_PASSWORD
  valueFrom:
    secretKeyRef:
      {{- if .Values.postgresql.enabled }}
      name: {{ template "gyyun.postgresql.fullname" . }}
      key: password
      {{- else if .Values.mysql.enabled }}
      name: {{ template "gyyun.mysql.fullname" . }}
      key: mysql-password
      {{- else }}
      name: {{ include "gyyun.fullname" . }}-externaldb
...
*/}}
{{- define "gyyun.security.env_vars" -}}
...
*/}}
{{- define "gyyun.database.wait-for-ready" -}}
- name: wait-for-database
  image: {{ .Values.initImage.busybox }}
  imagePullPolicy: {{ .Values.initImage.pullPolicy }}
{{- if .Values.postgresql.enabled }}
  command: ['sh', '-xc', 'for i in $(seq 1 180); do nc -z -w3 {{ template "gyyun.postgresql.fullname" . }} 5432 && exit 0 || sleep 5; done; exit 1']
{{- else if .Values.mysql.enabled }}
  command: ['sh', '-xc', 'for i in $(seq 1 180); do nc -z -w3 {{ template "gyyun.mysql.fullname" . }} 3306 && exit 0 || sleep 5; done; exit 1']
{{- else }}
...
*/}}
{{- define "gyyun.minio.wait-for-ready" -}}
{{- if .Values.minio.enabled }}
- name: wait-for-minio
  image: {{ .Values.initImage.busybox }}
  imagePullPolicy: {{ .Values.initImage.pullPolicy }}
  command: ['sh', '-xc', 'for i in $(seq 1 180); do nc -z -w3 {{ template "gyyun.minio.fullname" . }} 9000 && exit 0 || sleep 5; done; exit 1']
{{- end }}
{{- end -}}
...
*/}}
{{- define "gyyun.registry.env_vars" -}}
- name: REGISTRY_TYPE
...
- name: REGISTRY_HIKARI_CONFIG_PASSWORD
  valueFrom:
    secretKeyRef:
      name: {{ include "gyyun.fullname" . }}-registry-db
      key: registry-password
{{- end }}
{{- else }}
- name: REGISTRY_ZOOKEEPER_CONNECT_STRING
  {{- if .Values.zookeeper.enabled }}
  value: {{ template "gyyun.zookeeper.quorum" . }}
  {{- else }}
...
*/}}
{{- define "gyyun.sharedStorage.volume" -}}
{{- if .Values.common.sharedStoragePersistence.enabled -}}
- name: {{ include "gyyun.fullname" . }}-shared
  persistentVolumeClaim:
    claimName: {{ include "gyyun.fullname" . }}-shared
{{- end -}}
{{- end -}}
...
*/}}
{{- define "gyyun.sharedStorage.volumeMount" -}}
{{- if .Values.common.sharedStoragePersistence.enabled -}}
...
*/}}
{{- define "gyyun.fsFileResource.volume" -}}
{{- if .Values.common.fsFileResourcePersistence.enabled -}}
- name: {{ include "gyyun.fullname" . }}-fs-file
  persistentVolumeClaim:
    claimName: {{ include "gyyun.fullname" . }}-fs-file
{{- end -}}
{{- end -}}
...
*/}}
{{- define "gyyun.fsFileResource.volumeMount" -}}
{{- if .Values.common.fsFileResourcePersistence.enabled -}}
- mountPath: {{ default "/gyyun" .Values.common.configmap.RESOURCE_UPLOAD_PATH | quote }}
  name: {{ include "gyyun.fullname" . }}-fs-file
{{- end -}}
{{- end -}}
...
*/}}
{{- define "gyyun.etcd.ssl.volume" -}}
{{- if .Values.registryEtcd.ssl.enabled -}}
- name: etcd-ssl
  secret:
    secretName: {{ include "gyyun.fullname" . }}-etcd-ssl
{{- end -}}
{{- end -}}
...
*/}}
{{- define "gyyun.etcd.ssl.volumeMount" -}}
{{- if .Values.registryEtcd.ssl.enabled -}}
- mountPath: /opt/gyyun/{{ .Values.registryEtcd.ssl.certFile }}
  name: etcd-ssl
  subPath: cert-file
- mountPath: /opt/gyyun/{{ .Values.registryEtcd.ssl.keyCertChainFile  }}
  name: etcd-ssl
  subPath: key-cert-chain-file
- mountPath: /opt/gyyun/{{ .Values.registryEtcd.ssl.keyFile }}
  name: etcd-ssl
  subPath: key-file
{{- end -}}
{{- end -}}
...
*/}}
{{- define "gyyun.ldap.ssl.volume" -}}
{{- if .Values.security.authentication.ldap.ssl.enable -}}
- name: jks-file
  secret:
    secretName: {{ include "gyyun.fullname" . }}-ldap-ssl
{{- end -}}
{{- end -}}
...
*/}}
{{- define "gyyun.ldap.ssl.volumeMount" -}}
{{- if .Values.security.authentication.ldap.ssl.enable -}}
...
Usage:
{{ include "gyyun.tplvalues.render" ( dict "value" .Values.path.to.the.Value "context" $) }}
*/}}
{{- define "gyyun.tplvalues.render" -}}
