...
- The server-load-protection thresholds are consumed by `gyyun-worker` (`WorkerServerLoadProtection`). Changes to the interface here cascade to worker-side overrides.
- Prometheus scrape endpoint is at `/actuator/prometheus` (Spring Boot Actuator default). Auth is applied by `gyyun-actuator-authentication` when present on the classpath.
...
- `gyyun-common` — compile dep.
- `gyyun-actuator-authentication` (in `gyyun-authentication/`) — secures the actuator endpoints this module exposes.
