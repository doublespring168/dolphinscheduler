...
- `server.worker.executor` — the bridge to `gyyun-task-executor`. `PhysicalTaskEngineDelegator`, `PhysicalTaskExecutorFactory`, `PhysicalTaskExecutorLifecycleEventReporter`.
- `server.worker.rpc` — `WorkerRpcServer` + operators implementing `IPhysicalTaskExecutorOperator`, `IStreamingTaskInstanceOperator`, `ITaskExecutorQueryClient` (all from `gyyun-extract-worker`).
...
- **AWS S3 integration is direct** (via `gyyun-storage-s3`) for distributed-task artifact handling. Credentials follow `gyyun-aws-authentication` rules.
- **`gyyun-yarn-aop`** is on the worker's classpath: it weaves `YarnClientImpl.submitApplication` to capture `ApplicationId`s into `appInfo.log`. If YARN-based task plugins (MR, Spark-on-YARN) lose their application ID, check AspectJ weaving is active.
...
- **Load protection thresholds live in `application.yaml`**, **not** in `gyyun-meter`. Meter defines the interface; worker owns the numbers.
...
`src/test/java` — unit tests for registry, RPC, load protection, connection-state handling. End-to-end workflows are exercised by `gyyun-e2e` and the master's integration tests.

## Related modules

- `gyyun-task-executor` — the lifecycle framework.
- `gyyun-task-plugin` — the actual task implementations (runtime classpath via `gyyun-task-all`).
- `gyyun-extract-worker` — RPCs this module implements.
- `gyyun-extract-master` / `-extract-alert` — RPCs this module calls.
- `gyyun-yarn-aop` — AspectJ weaving on the classpath.
- `gyyun-registry-all`, `gyyun-storage-api`, `gyyun-datasource-api`.
